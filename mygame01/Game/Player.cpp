...
void Player::Move()
{
	...
	{
		//距離
		m_distance = m_enemy->m_position - m_position;
	}
	// 1P VS 2P のとき
	else if (m_game->m_2playerFlag == true)
	{
		//距離
		m_distance = m_player2->m_position - m_position;
	}
		...
		)
	{
		...
		if (m_charaCon.IsOnGround())
		{
			if (m_distance.Length() <= 20.0f)
			{
				...
				{
					//敵が画面端に行っていないか？
					if (m_enemy->m_noMove != true)
					{
						//プレイヤーが右側にいるとき
						if (m_enemy->m_position.x < m_position.x)
						{
							//敵を左に押す
							m_enemy->m_position.x -= 2.5f;
							m_enemy->m_charaCon.SetPosition(m_enemy->m_position);
							m_enemy->m_enemy.SetPosition(m_enemy->m_position);
						}
						//プレイヤーが左側にいるとき
						else
						{
				...
				{
					//プレイヤー2が画面端に行っていないか？
					if (m_player2->m_noMove != true)
					{
						//プレイヤーが右側にいるとき
						if (m_player2->m_position.x < m_position.x)
						{
							//プレイヤー2を左に押す
							m_player2->m_position.x -= 2.5f;
							m_player2->m_charaCon.SetPosition(m_player2->m_position);
							m_player2->m_player2.SetPosition(m_player2->m_position);
						}
						//プレイヤーが左側にいるとき
						else
						{
	...
	else
	{
		...
		if (m_charaCon.IsOnGround())
		{
			if (m_distance.Length() <= 20.0f)
			{
				...
				if (m_game->m_2playerFlag != true)
				{
					//以下の条件になっていたら敵が画面端まで行ってプレイヤーが敵に向けてジャンプしたとき反発する
					if (m_enemy->m_noMove == true && //敵が画面端に行ったか？
						...
						)
					{
						...
						{
							//右へ反発
							m_position.x = m_enemy->m_position.x + 20.0f;
						...
						)
					{
						...
						distance.x = m_position.x - m_enemy->m_position.x;
						m_enemy->m_position.x = m_enemy->m_position.x + (move.x + distance.x);
						m_enemy->m_charaCon.SetPosition(m_enemy->m_position);
						m_enemy->m_enemy.SetPosition(m_enemy->m_position);
						m_moveFlag = true;
						m_moveJumpFlag = false;
				...
				{
					//以下の条件になっていたらプレイヤー2が画面端まで行ってプレイヤーがプレイヤー2に向けてジャンプしたとき反発する
					if (m_player2->m_noMove == true && //プレイヤー2が画面端に行ったか？
						m_moveFlag != true && //剛体が移動していないか？
						m_moveJumpFlag == true //ジャンプしているか？
						)
					{
						...
						{
							//右へ反発
							m_position.x = m_player2->m_position.x + 20.0f;
							m_charaCon.SetPosition(m_position);
							m_player.SetPosition(m_position);
							m_moveFlag = true;
							m_moveJumpFlag = false;
						...
						)
					{
						Vector3 distance = Vector3::Zero;
						Vector3 move = m_repulsion;
						distance.x = m_position.x - m_player2->m_position.x;
						m_player2->m_position.x = m_player2->m_position.x + (move.x + distance.x);
						m_player2->m_charaCon.SetPosition(m_player2->m_position);
						m_player2->m_player2.SetPosition(m_player2->m_position);
						m_moveFlag = true;
						m_moveJumpFlag = false;
		...
		else if (m_charaCon.IsOnGround() == false)
		{
			...
			if (m_distance.Length() <= 20.0f)
			{
				...
				if (m_game->m_2playerFlag != true)
				{
					//敵が画面端まで行っていないときプレイヤーが敵に向けてジャンプしたとき反発する
					if (m_enemy->m_noMove != true &&
				...
				{
					//プレイヤー2が画面端まで行っていないときプレイヤーがプレイヤー2に向けてジャンプしたとき反発する
					if (m_player2->m_noMove != true &&
						fabsf(m_position.x) <= 530.0f //画面端から離れているか？
						)
					{
	...
	if (m_charaCon.IsOnGround())
	{
		...
		if (m_panchAnimationFlag != true && m_kickAnimationFlag != true)
		{
			...
			if (m_moveDirectionState == enMoveState_Right)
			{
				...
				m_frameMemoryRight = m_frame->m_playerframe[1];
			...
			else if (m_moveDirectionState == enMoveState_Left)
			{
				...
				m_frameMemoryLeft = m_frame->m_playerframe[4];
			...
			else
			{
				...
				if (m_startFlag == true)
				{
					...
					if (m_pushRightFlag == true)
					{
						...
						{
							//10F以内に
							if (m_frame->m_playerframe[0] < m_frame->GetFrame(10.0f))
							{
								m_stepFlagRight = 1;
							}
							else
...
void Player::FrameManage()
{
	...
	{
		m_frame->m_playerMoveState = Frame::EnDirectionState::enDirectionState_Down;
		//m_frame->m_state = 8;
		m_frame->m_playertenkey = 2;
	...
	{
		m_frame->m_playerMoveState = Frame::EnDirectionState::enDirectionState_Up;
		//m_frame->m_state = 7;
		m_frame->m_playertenkey = 8;
	...
	{
		m_frame->m_playerMoveState = Frame::EnDirectionState::enDirectionState_Left_Down;
		//m_frame->m_state = 6;
		m_frame->m_playertenkey = 1;
	...
	{
		m_frame->m_playerMoveState = Frame::EnDirectionState::enDirectionState_Left_Up;
		//m_frame->m_state = 5;
		m_frame->m_playertenkey = 7;
	...
	{
		m_frame->m_playerMoveState = Frame::EnDirectionState::enDirectionState_Left;
		//m_frame->m_state = 4;
		m_frame->m_playertenkey = 4;
	...
	{
		m_frame->m_playerMoveState = Frame::EnDirectionState::enDirectionState_Right_Down;
		//m_frame->m_state = 3;
		m_frame->m_playertenkey = 3;
	...
	{
		m_frame->m_playerMoveState = Frame::EnDirectionState::enDirectionState_Right_Up;
		//m_frame->m_state = 2;
		m_frame->m_playertenkey = 9;
	...
	{
		m_frame->m_playerMoveState = Frame::EnDirectionState::enDirectionState_Right;
		//m_frame->m_state = 1;
		m_frame->m_playertenkey = 6;
		m_moveDirectionState = enMoveState_Right;
	}
	//変更済_〇
	else //0:待機
	{
		m_frame->m_playerMoveState = Frame::EnDirectionState::enDirectionState_Idle;
		//m_frame->m_state = 0;
		m_frame->m_playertenkey = 5;
...
{	
	...
	if (m_game->m_2playerFlag != true)
	{
		...
		if (m_charaCon.IsOnGround() != false && //地面についている
			m_enemy->m_noMove != true //画面端まで行っていない
			)
		{
	...
	{
		//以下の条件になっていたら方法を固定しない
		if (m_charaCon.IsOnGround() != false && //地面に付いている
			m_player2->m_noMove != true //画面端まで行っていない
			)
		{
			m_rotationKeepFlag = false; //方向を固定しない
		}
	...
	else if (m_game->m_2playerFlag == true)
	{
		...
		if (m_player2->m_position.x < m_position.x)
		{
			...
			if (m_rotationKeepFlag != true)
			{
				...
				{
					m_rotationLeftFlag = true;
					m_rotationRightFlag = false;
					m_rotationKeepFlag = true;
				}
			}
		}
		//プレイヤーが左側にいるとき
		else
		{
			...
			if (m_rotationKeepFlag != true)
			{
				//スティックの方向が以下の方向に向いていなかったら
				if (m_moveDirectionState != enMoveState_Right_Up && //右上
					m_moveDirectionState != enMoveState_Up //左上
					)
				{
					m_rotationRightFlag = true;
...
void Player::Collision()
{
	...
	//上段
	const auto& panchCollisionE = g_collisionObjectManager->FindCollisionObject("enemy_panch_attack");
	const auto& flyingPanchCollisionE = g_collisionObjectManager->FindCollisionObject("enemy_panch_jump_attack");
	const auto& flyingKickCollisionE = g_collisionObjectManager->FindCollisionObject("enemy_kick_jump_attack");
	const auto& panchCollisionP2 = g_collisionObjectManager->FindCollisionObject("player2_panch_attack");
	const auto& flyingPanchCollisionP2 = g_collisionObjectManager->FindCollisionObject("player2_panch_jump_attack");
	const auto& flyingKickCollisionP2 = g_collisionObjectManager->FindCollisionObject("player2_kick_jump_attack");
	if ((panchCollisionE->IsHit(m_charaCon) || panchCollisionP2->IsHit(m_charaCon)) || //パンチ
		(flyingPanchCollisionE->IsHit(m_charaCon) || flyingPanchCollisionP2->IsHit(m_charaCon)) || //空中パンチ
		(flyingKickCollisionE->IsHit(m_charaCon) || flyingKickCollisionP2->IsHit(m_charaCon)) //空中キック
	   )
	{
		// 1P VS 2P ではないとき
		if (m_game->m_2playerFlag != true)
	...
	//下段
	const auto & kickCollisionE = g_collisionObjectManager->FindCollisionObject("enemy_kick_attack");
	const auto & kickCollisionP2 = g_collisionObjectManager->FindCollisionObject("player2_kick_attack");
	if (kickCollisionE->IsHit(m_charaCon) || kickCollisionP2->IsHit(m_charaCon) //キック
		)
	{
		// 1P VS 2P ではないとき
		if (m_game->m_2playerFlag != true)
	...
	if (m_charaCon.IsOnGround())
	{
			...
			)
		{
			if (g_pad[m_gamepadNo]->IsTrigger(enButtonA))
			{
				...
				//m_charaCon.SetRadiusAndHeight(10.0f,25.0f);
				////デバッグ用
				//m_playerHP =
		...
		//if (m_game->m_2playerFlag != true)
		//{
		//	if ((m_enemy->m_upGuardFlag == true || m_enemy->m_upDamageFlag == true) ||
		//		(m_enemy->m_downGuardFlag == true || m_enemy->m_downDamageFlag == true)
		//		)
		//	{
		//		return;
		//	}
		...
		//else if (m_game->m_2playerFlag == true)
		//{
		//	if ((m_player2->m_upGuardFlag == true || m_player2->m_upDamageFlag == true) ||
		//		(m_player2->m_downGuardFlag == true || m_player2->m_downDamageFlag == true)
		//		)
		//	{
		//		return;
		//	}
		...
		{
			//敵の当たり判定が当たったら処理しない
			if (m_enemy->m_collisionHitFlag == true)
			{
				return;
			}
		}
		...
		{
			//プレイヤー2の当たり判定が当たったら処理しない
			if (m_player2->m_collisionHitFlag == true)
			{
				return;
			}
		}
...
void Player::MakePanchAttackCollision()
{
	//コリジョンオブジェクトを作成する。
	...
	collisionObject->SetName("player_panch_attack");
}

void Player::MakeFlyingPanchAttackCollision()
{
	//コリジョンオブジェクトを作成する。
	auto collisionObject = NewGO<CollisionObject>(0);
	Vector3 collisionPosition = m_position;
	...
	collisionObject->SetName("player_panch_jump_attack");
}

void Player::MakeKickAttackCollision()
{
	//コリジョンオブジェクトを作成する。
	auto collisionObject = NewGO<CollisionObject>(0);
	Vector3 collisionPosition = m_position;
	...
	collisionObject->SetName("player_kick_attack");
}

void Player::MakeFlyingKickAttackCollision()
{
	//コリジョンオブジェクトを作成する。
	auto collisionObject = NewGO<CollisionObject>(0);
	Vector3 collisionPosition = m_position;
	...
	collisionObject->SetName("player_kick_jump_attack");
}
...
void Player::ManageState()
{
		...
		)
	{
		...
		if (m_game->m_2playerFlag != true)
		{
			...
			//時間切れ時の負け
			else if (m_playerHP <= m_enemy->m_enemyHP)
			{
		...
		else if (m_game->m_2playerFlag == true)
		{
			...
			//時間切れ時の負け
			else if (m_playerHP <= m_player2->m_playerHP)
			{
				m_animationState = enAnimationState_Lose_TimeUp;
				return;
			}
	...
	else
	{
		...
		else if (m_round->m_roundState == Round::EnRoundState::State_KO)
		{
			...
			//{
			//	m_player.SetAnimationSpeed(0.0f);
			//}
			if (m_playerHP <= 0)
			{
				m_animationState = enAnimationState_Lose;
		...
		if (m_charaCon.IsOnGround() == false)
		{
			...
			else
			{
				...
				if (m_game->m_2playerFlag != true)
				{
					if (m_enemy->m_animationState == Enemy::EnAnimationState::enAnimationState_Damage)
					{
						m_enemy->m_collisionHitFlag = false;
					}
				}
				// 1P VS 2P のとき
				else if (m_game->m_2playerFlag == true)
				{
					if (m_player2->m_animationState == Player2::EnAnimationState::enAnimationState_Damage)
					{
						m_player2->m_collisionHitFlag = false;
					}
				}
				return;
			}
...
void Player::PlayAnimation()
{
	...
	case enAnimationState_Idle: //待機
		m_player.PlayAnimation(enAnimationClip_Idle, 0.1);
		break;
	case enAnimationState_Jump: //ジャンプ
		m_player.SetAnimationSpeed(0.8f);
		m_player.PlayAnimation(enAnimationClip_Jump);
		break;
	case enAnimationState_Walk: //歩き
		...
		if (m_game->m_2playerFlag != true)
		{
			...
			if (m_enemy->m_position.x < m_position.x)
			{
				...
				if (m_moveDirectionState == enMoveState_Right)
				{
					m_player.PlayAnimation(enAnimationClip_Walk_Back);
				}
				//else if (m_stickL.x < 0.0f)
				//変更済
				else if (m_moveDirectionState == enMoveState_Left)
				{
					m_player.PlayAnimation(enAnimationClip_Walk_Forward);
				}
			}
			//プレイヤーが左側にいるとき
			else
	...
	case enAnimationState_Step: //ステップ
		m_player.SetAnimationSpeed(2.5f);
		// 1P VS 2P ではないとき
		if (m_game->m_2playerFlag != true)
		{
			//プレイヤーが右側にいるとき
			if (m_enemy->m_position.x < m_position.x)
			{
				...
				{
					m_player.PlayAnimation(enAnimationClip_Step_Back);
				}
				else if (m_stepAnimationFlagLeft == true)
				{
					m_player.PlayAnimation(enAnimationClip_Step_Forward);
				}
			}
			//プレイヤーが左側にいるとき
			else
	...
	case enAnimationState_Panch: //パンチ
		if (m_charaCon.IsOnGround() != true)
		{
			m_player.SetAnimationSpeed(1.5f);
			m_player.PlayAnimation(enAnimationClip_Panch_Jump, 0.1);
		}
		else
		{
			m_player.PlayAnimation(enAnimationClip_Panch, 0.1);
		}
		break;
	case enAnimationState_Kick: //キック
		...
		{
			m_player.SetAnimationSpeed(1.5f);
			m_player.PlayAnimation(enAnimationClip_Kick_Jump, 0.1);
		}
		else
		{
			m_player.PlayAnimation(enAnimationClip_Kick, 0.1);
		}
		break;
	case enAnimationState_Guard: //ガード
		if (m_upGuardFlag == true)
		{
			m_player.PlayAnimation(enAnimationClip_Panch_Guard, 0.1);
		}
		else if (m_downGuardFlag == true)
		{
			m_player.PlayAnimation(enAnimationClip_Kick_Guard, 0.1);
		}
		break;
	case enAnimationState_Damage: //ダメージ
		if (m_upDamageFlag == true)
		{
			m_player.PlayAnimation(enAnimationClip_Panch_Damage, 0.1);
		}
		else if (m_downDamageFlag == true)
		{
			m_player.PlayAnimation(enAnimationClip_Kick_Damage, 0.1);
		}
		else if (m_flyDamageFlag == true)
		{
			m_player.PlayAnimation(enAnimationClip_Fly_Damage, 0.1);
		}
		break;
	case enAnimationState_Win: //勝ち
		m_player.PlayAnimation(enAnimationClip_Win);
		break;
	case enAnimationState_Lose: //負け
		m_player.PlayAnimation(enAnimationClip_Lose);
		break;
	case enAnimationState_Lose_TimeUp: //負け
		m_player.PlayAnimation(enAnimationClip_Lose_TimeUp);
		break;
	case enAnimationState_Draw: //引き分け
		m_player.PlayAnimation(enAnimationClip_Draw);
		break;
	default:
		break;
	}
